...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        def get_symbol_option(word, id):
            #check all of the symbols
            #first check is for root
            if word == None:
                return self.word_vocab['<ROOT>']
            word = word.lower()
            #second two checks for NNP or CD
            if pos[id] == 'NNP':
                return self.pos_vocab[pos[id]]
            elif pos[id] == 'CD':
                return self.pos_vocab[pos[id]]
            #case if it is not in vocab then it is an unkown
            elif not self.word_vocab.get(word):
                return self.word_vocab['<UNK>']
            #none of the special chars so has to be a word in vocab
            else:
        ...
        for i in range(1,4):
            ...
            if len(state.stack) >= i:
                #go through the stack and check all words
                negative_i = -i
                curr_id = state.stack[negative_i]
                curr_word = words[curr_id]
                word = get_symbol_option(curr_word, curr_id)
                vector[current] = word

            #empty stack - set vector[current] to null
            else:
                vector[current] = self.word_vocab['<NULL>']
            #increment current
            current += 1
        ...
        for i in range(1, 4):
            ...
            if len(state.buffer) >= i:
                # go through the stack and check all words
                negative_i = -i
                curr_id = state.buffer[negative_i]
        ...
        #once going through stack and buffer return the vector
        #print("vector: ",  vector)
        #print("words: ", words)
    ...
    def get_output_representation(self, output_pair):
        # TODO: Write this method for Part 2
        num_outputs = 91
        output_labels = self.make_output_labels()
        output_index = output_labels[output_pair]
        return keras.utils.to_categorical(output_index, num_classes=num_outputs)
